...
C
C     This version includes basal sliding.
C
...
C
      ...
      DIMENSION UX(201),US(201),RATIO(201)
C
C
C---  Input parameters
C
...
C
      ...
      CSLIDE=CON2/((2.0*DX)**2.0)
      OPEN(2,FILE='SSHAPEBS.CVV',STATUS='NEW')
      OPEN(4,FILE='TSHAPEBS.CVV',STATUS='NEW')
      OPEN(3,FILE='SSHASTBS.CVV',STATUS='OLD')
C
C
C---  Initial geometry.
C
...
C
      DO 100 I=2,IMAX 
         ...
         ELSE
            DHSDX=HS(I+1)-HS(I-1)
            DIFDEF=CDIFF*H(I)*H(I)
            HBUOY=H(I)+((ROW/ROI)*HB(I))+1.0
            IF(HBUOY.LE.0.0) THEN
               DIFSL=0.0
            ELSE
               DIFSL=(CSLIDE/HBUOY)*H(I)
            ENDIF
            DIFF(I)=((DIFDEF+DIFSL)*DHSDX*DHSDX)*(H(I)**3.0)
            RATIO(I)=DIFSL/(DIFDEF+DIFSL)
         ENDIF
100   CONTINUE
      DIFF(1)=0.0
C
...
265   CONTINUE
      ...
      X1=(DHSDX*DHSDX*DHSDX)*(H(IGR)**3.0)
      HBUOY=H(IGR)+((ROW/ROI)*HB(IGR))+1.0
      UGR=X1*((H(IGR)*CONST)+(CON2/HBUOY))
      UGR=-UGR
C
C
      X1=CSHELF/ACC
...
280      CONTINUE
         VOL=VOL*DX
         UTOT=-(FLUX(21)+FLUX(20))*(0.5/H(21))
         US(21)=UTOT*RATIO(21)
         UX(21)=UTOT*(1.0-RATIO(21))
         UX(31)=0.0
         US(31)=0.0
...
C
      DO 290 I=2,IMAX
         IF(H(I).EQ.0.0) THEN
            ...
            US(I)=0.0
         ELSE
            UTOT=-(FLUX(I)+FLUX(I-1))*(0.5/H(I))
            US(I)=UTOT*RATIO(I)
            UX(I)=UTOT*(1.0-RATIO(I))
         ENDIF
290   CONTINUE
C
C
