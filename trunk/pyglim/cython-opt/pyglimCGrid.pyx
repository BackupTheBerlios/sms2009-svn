...
def dfdx_2d_stag(np.ndarray[np.float64_t, ndim=2] f, Py_ssize_t i, Py_ssize_t j, np.float64_t delta):
    return (f[i+1, j] + f[i+1, j+1] - f[i, j] - f[i, j+1])/(2.0*delta) 

def dfdy_2d_stag(np.ndarray[np.float64_t, ndim=2] f, Py_ssize_t i, Py_ssize_t j, np.float64_t delta):
